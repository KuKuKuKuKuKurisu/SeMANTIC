...
class DST_AWARE(nn.Module):
    ...
    def turn_level_self_attn(self, context_txt_emb, context_img_emb, turn_mask, state_embedding):
    ...
    def forward(self, context_dialog, pos_products, neg_products, eval = False):
        ...
        for turn_id in range(DatasetConfig.dialog_context_size):
            context_text_embedding, context_image_embedding = self.get_context_emb(text_ids[:, turn_id, :],
                                                                                   images[:, turn_id, :, :, :, :], image_mask[:, turn_id, :])
        ...
        for neg_id in range(DatasetConfig.max_neg_num):
            ...
            neg_text_embedding, neg_image_embedding = self.get_item_embedding(neg_attrs_id[:, neg_id, :],
                                                                              neg_imgs[:, neg_id, :, :, :],
                                                                              state_embedding)
            # neg sim
            ...
            if eval == False:
                ...
                neg_text_image_sims.append(torch.max(zeros, RecommendTrainConfig.txt_img_threshold - neg_text_image_sim))
                # neg kl
        ...
        for pos_id in range(DatasetConfig.max_pos_num):
            ...
            pos_text_embedding, pos_image_embedding = self.get_item_embedding(pos_attrs_id[:, pos_id, :],
                                                                              pos_imgs[:, pos_id, :, :, :],
                                                                              state_embedding)
            # pos sim
            ...
            if eval == False:
                ...
                pos_text_image_sims.append(torch.max(zeros, RecommendTrainConfig.txt_img_threshold - pos_text_image_sim))
                # pos kl
