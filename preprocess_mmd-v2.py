...
from config import DatasetConfig, RecommendTrainConfig, constants
...
def main():
    ...
    #common data
    splits = ['train', 'fewshot', 'valid', 'test']
    ...
    if not isfile(DatasetConfig.wodst_common_raw_data_file):
        ...
        save_pkl(common_data, 'common_data',
                 DatasetConfig.wodst_common_raw_data_file)

    for split in splits:

        if split == 'train':
            input_paths = [DatasetConfig.wodst_train_dialog_data_directory, DatasetConfig.wodst_fewshot_dialog_data_directory]
            raw_output_path = DatasetConfig.wodst_train_raw_data_file
            tidy_output_path = DatasetConfig.wodst_recommend_train_dialog_file
        elif split == 'fewshot':
            input_paths = [DatasetConfig.wodst_fewshot_dialog_data_directory]
            raw_output_path = DatasetConfig.fewshot_raw_data_file
            tidy_output_path = DatasetConfig.recommend_fewshot_dialog_file
        elif split == 'valid':
            input_paths = [DatasetConfig.wodst_valid_dialog_data_directory]
            raw_output_path = DatasetConfig.wodst_valid_raw_data_file
            tidy_output_path = DatasetConfig.wodst_recommend_valid_dialog_file
        else:
            input_paths = [DatasetConfig.wodst_test_dialog_data_directory]
            raw_output_path = DatasetConfig.wodst_test_raw_data_file
            tidy_output_path = DatasetConfig.wodst_recommend_test_dialog_file
        ...
        if not has_raw_data_pkl:
            ...
            dialogs = []
            for input_path_id, input_path in enumerate(input_paths):
                for file_id, file in enumerate(tqdm(os.listdir(input_path))):
                    ...
                    if (split=='train') & (input_path_id==0):
                        if len(dialogs) >= (105439*RecommendTrainConfig.wodst_data_proportion):
                            break
